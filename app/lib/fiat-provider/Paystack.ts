...
import { PROCESS_TYPES, supportedChains, transactionStatus, transactionType, userPaymentType } from 'App/helpers/types';
...
interface PaystackResponse {
  ...
  export default class Paystack implements IPaymentProvider {
    ...
    public async initializePayment(txRef: string, amount: string, email: string, paymentType: string): Promise<{ status: string; data: BaseResponseData }> {
      try {
        const details = {
          reference: txRef,
          ...
          channels: paymentType === userPaymentType.BANK_TRANSFER ? ['bank_transfer'] : ['card'],
          metadata: {
        ...
        const response = await this.sdk.transaction.initialize(details) as PaystackResponse;
        ...
        if (paymentType === userPaymentType.BANK_TRANSFER && response.data.transfer_details) {
          baseResponse.data = {
            ...baseResponse.data,
  ...
  public async generateBankAccount(txRef: string, amount: string, email: string): Promise<any> {
    return this.initializePayment(txRef, amount, email, userPaymentType.BANK_TRANSFER);
  }
  ...
  public async initSendBankTransfer({ accountBank, accountNumber, amount, txRef }: params): Promise<any> {
    try {
      ...
      const transferDetails = {
        ...
        recipient: await this.createTransferRecipient(accountNumber, accountBank),
        reason: "WT Payment",
        currency: "NGN",
        reference: txRef,
      };
  ...
  private async createTransferRecipient(accountNumber: string, bankCode: string): Promise<string> {
    ...
    if (!response.status) throw new Error('Failed to create recipient: ' + response.message);
  ...
  public async processWebhook({ request, response }: HttpContextContract) {
    try {
      ...
      const verifyResponse = await this.sdk.transaction.verify(payload.data.reference);
