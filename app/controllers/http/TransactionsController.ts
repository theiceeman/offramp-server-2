...
export default class TransactionsController extends RolesController {
  public async createOfframpCrypto({
    request,
    response,
    auth,
  }: HttpContextContract) {
    try {
      ...
      if (
        amountInUsd < setting.minTransactionAmount ||
        amountInUsd > setting.maxTransactionAmount
      )
        throw new Error(
          `Transaction limit is between ${setting.minTransactionAmount.toLocaleString()} and ${setting.maxTransactionAmount.toLocaleString()} USD.`
        );
      ...
      let transaction =
        await new TransactionUtils()._calculateRatesForUserSelling(
          senderCurrencyId,
          recieverCurrencyId,
          amountInUsd,
          transactionType.CRYPTO_OFFRAMP,
          "sending"
        );

      let txnWalletAddress =
        await new PaymentProvidersController().processCryptoPaymentMethod(
          uniqueId,
          senderCurrencyId
        );
   ...
   * @param param0
   */
  public async validateOffRampCrypto({
    request,
    response,
    auth,
  }: HttpContextContract) {
    try {
      ...
      let result = await new TransactionUtils()._calculateRatesForUserSelling(
        senderCurrencyId,
        recieverCurrencyId,
        amountInUsd,
        transactionType.CRYPTO_OFFRAMP,
      ...
      response
        .status(200)
        .json({ data: result, message: "Request successfull." });
    } catch (error) {
      response.status(400).json(await formatErrorMessage(error));
    }
  }
   ...
   * @param param0
   */
  public async validateBuyCrypto({
    request,
    response,
    auth,
  }: HttpContextContract) {
    try {
      ...
      let result = await new TransactionUtils()._calculateRatesForUserBuying(
        senderCurrencyId,
        recieverCurrencyId,
        amountInUsd,
  ...
  public async createBuyCrypto({
    ...
    try {
      ...
      let transaction =
        await new TransactionUtils()._calculateRatesForUserBuying(
          senderCurrencyId,
          recieverCurrencyId,
          amountInUsd,
          transactionType.BUY_CRYPTO,
      ...
      let { fiatProviderTxRef, paymentDetails } =
        await new PaymentProvidersController().processSelectedFiatPaymentMethod(
          recieverCurrencyId,
          paymentType,
          transaction[0].actual_amount_user_sends,
          uniqueId
        );
   ...
   * @param param0
   */
  public async viewLoggedInUserTransactions({
    response,
    auth,
  }: HttpContextContract) {
    try {
   ...
   * @param param0
   */
  public async viewSingleTransaction({
    response,
    auth,
    params,
  }: HttpContextContract) {
    try {
      ...
      let transaction = await Transaction.query()
        ...
        .preload("sendingCurrency", (query) =>
          query.select("name", "network", "symbol", "market_usd_rate")
        )
        ...
        .where((query) => {
          query
            ...
            .orWhere("fiatProviderTxRef", String(params.id).toLowerCase());
        });

      let txnType: "userBuy" | "userSell" =
        transaction[0].type === transactionType.BUY_CRYPTO
          ? "userBuy"
          : "userSell";
      let actualAmountUserReceives = this._calcActualAmountUserRecieves(
        transaction,
        txnType
      );
      let actualAmountUserSends = this._calcActualAmountUserSends(
        transaction,
        txnType
      );
      let txnProfitFromRate =
        await new AppOverviewsController().calculateSingleTransactionProfitFromRate(
   ...
   * @param param0
   */
  public async setStatusProcessing({
    request,
    response,
    auth,
  }: HttpContextContract) {
    try {
      ...
      let transaction = await Transaction.query().where(
        "unique_id",
        data.txnId
      );
      ...
      let result = await Transaction.query()
        ...
        .update({
          ...
          processedBy: user.uniqueId,
        });

      if (result[0] > 0) {
        response
          .status(200)
          .json({ data: "Transaction, now set to processing." });
      } else {
        throw new Error("Action failed!");
      }
    } catch (error) {
   ...
   * @param param0
   */
  public async reverseStatusToConfirmed({
    request,
    response,
    auth,
  }: HttpContextContract) {
    try {
      ...
      if (result[0] > 0) {
        response
          .status(200)
          .json({ data: "Transaction released back into pool." });
      } else {
        throw new Error("Action failed!");
      }
    } catch (error) {
   ...
   * @param param0
   */
  public async setStatusComplete({
    request,
    response,
    auth,
  }: HttpContextContract) {
    try {
      ...
      if (result[0] > 0) {
        ...
        await new WebSocketsController().emitStatusUpdateToClient(data.txnId);
   ...
   * @returns
   */
  public _calcActualAmountUserRecieves(
    transaction: any,
    type: "userBuy" | "userSell"
  ) {
    ...
    if (type === "userBuy") {
      actualAmountUserReceives =
        amountInUsd / transaction[0].recievingCurrencyUsdRate;
  ...
  */
   ...
   * @returns
   */
  public _calcActualAmountUserSendsDuringValidation(
    transaction: any,
    type: "userBuy" | "userSell"
  ) {
    ...
    if (type === "userBuy") {
      actualAmountUserSends =
        amountInUsd * transaction[0].sendingCurrencyUsdRate;
    } else if (type === "userSell") {
      actualAmountUserSends =
        amountInUsd / transaction[0].sendingCurrencyUsdRate;
    }
   ...
   * @returns
   */
  public _calcActualAmountUserSends(
    transaction: any,
    type: "userBuy" | "userSell"
  ) {
    let actualAmountUserSends;
   ...
   */
  public async checkIfUserHasPendingTransaction(
    userUniqueId,
    sendingCurrencyId: string
  ) {
    try {
      let sendingCurrency = await Currency.query()
        .where("unique_id", sendingCurrencyId)
        .where("is_deleted", false);
      ...
      if (pendingTransaction.length > 0)
        throw new Error(
          `You have a pending ${sendingCurrency[0].symbol} transaction.`
        );
    } catch (error) {
